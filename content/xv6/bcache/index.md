+++
date = '2025-10-28T14:02:59+08:00'
draft = false
title = '[xv6 學習紀錄 08-3] bcache 相關程式碼解析'
series = ["xv6 學習紀錄"]
weight = 83
+++
...
main()
{
  if(cpuid() == 0){
    ...
    binit();         // buffer cache <- 這裡進入 bcache 的初始化過程
    iinit();         // inode table
    fileinit();      // file table
...
```
* 在 `binit()` 之後，就不會直接使用 `bcache.buf` 這個 array，而是使用由 `head` 開頭的這個環狀 linked-list
